...
function seedBlogFormatted( $inBlogName,
    ...
    $inNumToSkip = sb_inputFilter( $inNumToSkip, "/[0-9-]+/", "-1" );

    $limitNumber = sb_inputFilter( $inMaxNumber, "/[0-9-]+/", "-1" );
    ...
    for( $i=0; $i<$numRows; $i++ ) {
        ...
        if( $inShowIntroText ) {
            ...
            if( $inOrder == 2 && sb_isAdministrator() ) {
                ...
                $upShown = false;
                if( $index > 0 ) {
                    echo " [<A HREF=\"seedBlogs.php?action=move_top".
                        "&blog_name=$inBlogName".
                        "&post_id=$post_id&return_url=$return_url\">" .
                        "Top</A>]";
...
else if( strcmp( $action, "move_top" ) == 0 ) {
    sb_moveTop( $blog_name, $post_id );
    }
...
/**
 * Moves a post to the top of the order map.
 *
 * @param $inBlogName the name of the blog.
 * @param $inPostID the post to move up.
 */
function sb_moveTop( $inBlogName, $inPostID ) {

    // keep moving it up until it stops
    while( sb_movePost( $inBlogName, $inPostID, -1 ) ) {
 ...
 *
 * Returns true if post could move, or false if it could not
 */
...
function sb_rssFeed() {
    ...
    $limitNumber = sb_inputFilter( $max_number, "/[0-9-]+/", "-1" );
...
function sb_inputFilter( $inInput, $inRegex, $inDefault = "" ) {
